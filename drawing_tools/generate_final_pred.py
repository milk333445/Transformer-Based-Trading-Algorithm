...
def predict_stock_data(stock_symbol, save_directory, checkpoint, sequence_length=15, mode='val', visulize_attention=True, visulize_final_result=True):
    print('模式 : ', mode)
    all_results = []
    ...
    save_path = os.path.join(save_directory, f'{stock_symbol}_final_pred.csv')
    final_pred.to_csv(save_path)
...
if __name__ == "__main__":
    all_stocks = ["GOOGL", "^DJI", "^IXIC", "^N225", "^SOX", "^TWII", "AAPL", "AMZN", "ASML", "HPQ", "IBM", "INTC", "MSFT", "MU", "NFLX", "ORCL", "T", "TXN", "VZ"]
    all_stocks = ["^DJI"]
    parser = argparse.ArgumentParser()
    parser.add_argument('--save_directory', type=str, default='./final_preds')
    parser.add_argument('--checkpoint', type=str, default='vit_checkpoints')
    args = parser.parse_args()
    for symbol in all_stocks:
        print('目前處理股票代號 : ', symbol)
        predict_stock_data(symbol, args.save_directory, args.checkpoint, mode='val', visulize_attention=False, visulize_final_result=False)
